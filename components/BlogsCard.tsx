...
type BlogsCardProps = BlogsCardType & {
...
export default function BlogsCard(item: BlogsCardProps) {
  ...
  async function skipReplacement() {
    try {
      ...
      const res = await fetch("/api/blogs/skipReplacement", {
        method: "POST",
      ...
      router.push("/blogs/suggested-blogs");
    } catch (error) {
  ...
  async function addToBlog() {
    ...
    try {
      ...
      if (item?.index !== undefined) {
        dispatch(removeGeneratedBlog(item?.index));
      }
    } catch (error) {
    ...
    >
      ...
      {item?.inDetailPage && (
        <div className="w-full flex justify-start gap-[12px] items-center mb-[16px]">
          <Link href={"/blogs"} className="pl-2">
            <ChevronLeft />
          </Link>
          <h4
          ...
          <div className="hidden lg:flex justify-end items-center gap-[16px] w-fit">
            ...
            {!done && (
              <Button
                ...
                asChild
              >
                <Link href={`/blogs/getAdvice/${item._id}`}>Get Advice</Link>
              </Button>
            )}
