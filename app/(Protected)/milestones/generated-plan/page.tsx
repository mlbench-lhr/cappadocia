...
export default function GeneratedMilestones() {
  ...
  return (
    <div className="flex flex-col gap-[32px] justify-start items-start w-full">
      <div className="h-fit py-[24px] px-[0px] md:px-[40px] flex flex-col gap-[32px] justify-between items-end w-full relative">
        <Button
        ...
        {milestones?.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-1 gap-6 w-full">
            {milestones?.map((item, index) => (
              <MilestonesCard key={index} {...item} inPlanPage index={index} />
            ))}
          </div>
        ) : (
