...
export default function ExplorePage() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchQuery && searchQuery.trim().length > 0) {
      ...
      list = list.filter(
        (i) =>
          ...
          i.vendorDetails.title.toLowerCase().includes(q)
      );
    }
    ...
    if (priceFilter) {
      const { min, max } = priceFilter.priceRange;
      list = list.filter((it) => {
        if (min != null && max != null)
          return it.price >= min && it.price <= max;
        if (min != null) return it.price >= min;
        if (max != null) return it.price <= max;
        return true;
      });
    }
    ...
    if (ratingFilter) {
      const minRating = ratingFilter.rating;
      list = list.filter((it) => it.rating >= minRating);
    }
