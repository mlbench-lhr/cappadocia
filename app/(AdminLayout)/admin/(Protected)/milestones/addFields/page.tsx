...
export default function AddMilestoneField() {
  ...
  const [dialogType, setDialogType] =
    useState<keyof MilestoneFieldsType>("tier");
  ...
  useEffect(() => {
    async function getFields() {
      try {
        setLoading(true);
        const response = await fetch("/api/milestonesFields");
  ...
  const handleLoadMore = async (type: keyof MilestoneFieldsType) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/milestonesFields?field=${type}&offset=${currentOffset}&limit=${LOAD_MORE_COUNT}`
      );
    ...
    try {
      ...
      const allFieldsPromises = Object.keys(formData).map(async (fieldKey) => {
        ...
        if (currentLength < totalLength) {
          const response = await fetch(
            `/api/milestonesFields?field=${type}&offset=${currentLength}&limit=${
      ...
      if (response.ok) {
        Swal.fire(
          "Success",
          "Milestone fields updated successfully!",
          "success"
        );
      } else {
        Swal.fire("Error", "Failed to update milestone fields.", "error");
      }
    } catch (error) {
      console.error("Error updating milestone fields:", error);
      Swal.fire("Error", "Error updating milestone fields.", "error");
    } finally {
